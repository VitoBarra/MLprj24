...
class Optimizer:
    ...
    def update_weights(self, layer: Layer):
        ...
        while layer.NextLayer is not None:
            ...
            layer.WeightToNextLayer = layer.WeightToNextLayer - self.updates[-index]
            #print(f"from {layer.Name} to {layer.NextLayer.Name}: {layer.WeightToNextLayer}")
            if np.linalg.norm(self.updates[-index]) > 0.3 :
                print(f"update {layer.Name} to {layer.NextLayer.Name}: {self.updates[-index]}")
        ...
        if layer.LastLayer is None:
            ...
            if layer.NextLayer is None:
                ...
                for out, y, net in zip(layer.LayerOutput, target, layer.LayerNets):
                    self.deltas.append((self.loss_function.CalculateDerivLoss(out, y) *
                                        act_fun.CalculateDerivative(net)))

            # Hidden layer
            else:
                # transpose weight metric to use a single unit's output weights
                transposed_weight = layer.WeightToNextLayer.T
                prev_delta = np.array(self.deltas)
                self.deltas.clear()
                temp = []

                for weightFrom1Neuron_toAll, net_one_unit in zip(transposed_weight, layer.LayerNets.T):
                    ...
                    temp.append(delta)
                self.deltas = np.array(temp).T.tolist()
            ...
            for prev_o_one_pattern, deltas_one_pattern in zip(layer.LayerInput, deltas):
                ...
                d = np.expand_dims(deltas_one_pattern, axis=0)
                grad_from_one_unit = self.eta * (b @ d)
            ...
            if self.regularization is True:
                new_weights = layerGrad + (self.lambda_ * layer.LastLayer.WeightToNextLayer)
                #new_weights = layerGrad + (self.lambda_ / deltas.shape[0]) * layer.LastLayer.WeightToNextLayer
            else:
                new_weights = layerGrad

            self.updates.append(new_weights)
